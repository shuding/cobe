...
export function Page() {
  ...
  useEffect(() => {
    ...
    let lastMarkerSize = paramsRef.current.markerSize
    const p = create(canvas, {
      ...
      onRender: (state) => {
        ...
        if (lastMarkerSize !== paramsRef.current.markerSize) {
          lastMarkerSize = paramsRef.current.markerSize
          state.markers = INITIAL_MARKER.map((marker) => ({ ...marker, size: paramsRef.current.markerSize }))
        }
